...
<section class="login-wrapper">
  <div class="login-box">
    ...
    <form action='<?= PATH . "/admin/subCategory";?>' method="post">
      <ul>
        <div class="input account">
          ...
          <select name="category_id">
            ...
            <?php else:?>
              ...
              <?php $oldCategoryId = (int)SessionService::flash('old.category_id');?>
              <?php foreach($categories as $category) :?>
              <option value="<?= h($category['id']); ?>" <?= $oldCategoryId === $category['id'] ? 'selected': ''; ?>>
                <?= $category['name'];?>
              </option>
              <?php endforeach;?>
            <?php endif;?>
          </select>
          <span class="error-msg"><?= SessionService::flash('errors.category_id') ?></span>
        </div>
        <div class="input account">
          <div>
